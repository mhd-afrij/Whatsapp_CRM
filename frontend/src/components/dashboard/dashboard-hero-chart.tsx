...
export function DashboardHeroChart({ data, isLoading, isError }: DashboardHeroChartProps) {
  ...
  const totals = (data ?? []).reduce((result, point) => ({ won: result.won + point.won_value, lost: result.lost + point.lost_value }), { won: 0, lost: 0 });
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
      <div className="flex flex-col gap-5 border-b border-border p-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Revenue pulse</p>
          <h2 className="mt-1 text-xl font-bold tracking-tight text-text">Pipeline performance</h2>
          <p className="mt-1 text-sm text-muted">Won and lost deal value across the selected period.</p>
        </div>
        <div className="flex gap-5">
          <div><p className="text-xs text-muted">Won value</p><p className="mt-1 text-lg font-bold text-success">{formatCurrency(totals.won)}</p></div>
          <div><p className="text-xs text-muted">Lost value</p><p className="mt-1 text-lg font-bold text-danger">{formatCurrency(totals.lost)}</p></div>
        </div>
      </div>
        ...
        {isLoading ? <div className="h-52 animate-pulse rounded-xl bg-border/60" /> : isError ? <div className="flex h-52 items-center justify-center text-sm text-danger">Unable to load pipeline performance.</div> : !hasData ? <div className="flex h-52 items-center justify-center text-sm text-muted">No closed deal value for this period yet.</div> : (
          <ResponsiveContainer width="100%" height={208}>
            <AreaChart data={data ?? []} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="won_value" name="Won value" stroke="var(--chart-series-3)" fill="url(#wonValueGradient)" strokeWidth={2.5} />
