...
function ChartCard({ title, isLoading, isError, isEmpty, emptyLabel, onRetry, children }: { title: string; isLoading: boolean; isError?: boolean; isEmpty: boolean; emptyLabel: string; onRetry?: () => void; children: React.ReactNode }) {
  return <section className="rounded-2xl border border-border bg-surface p-4 shadow-sm sm:p-5"><h2 className="mb-3 text-sm font-bold text-text">{title}</h2>{isLoading ? <div className="h-44 animate-pulse rounded-xl bg-border/60" /> : isError ? <div className="flex h-44 flex-col items-center justify-center gap-2 text-sm text-danger"><span>Unable to load {emptyLabel}.</span>{onRetry && <button type="button" onClick={onRetry} className="text-xs font-semibold text-primary hover:underline">Retry</button>}</div> : isEmpty ? <div className="flex h-44 items-center justify-center text-sm text-muted">No {emptyLabel} yet for this period.</div> : children}</section>;
}
...
function DashboardContent() {
  ...
  return <div className="space-y-5 pb-4">
    ...
    {summary.isLoading ? <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-32 animate-pulse rounded-2xl bg-border/60" />)}</div> : summary.isError ? <div className="flex items-center gap-2 rounded-xl border border-danger/30 bg-danger-light/30 p-4 text-sm text-danger"><AlertCircle className="size-4" />Unable to load dashboard summary.</div> : s && <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
      ...
      <section className="rounded-2xl border border-border bg-surface p-4 shadow-sm sm:p-5"><div className="flex items-center gap-2"><Activity className="size-4 text-primary" /><h2 className="text-sm font-bold text-text">Task completion</h2></div>{taskCompletion.isLoading ? <div className="mt-4 h-2 animate-pulse rounded-full bg-border/60" /> : taskCompletion.data && taskCompletion.data.total > 0 ? <><div className="mt-4 h-2 overflow-hidden rounded-full bg-border"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${taskCompletion.data.rate_percent}%` }} /></div><div className="mt-2 flex items-end justify-between"><p className="text-2xl font-bold text-text">{taskCompletion.data.rate_percent}%</p><p className="text-xs text-muted">{taskCompletion.data.completed} of {taskCompletion.data.total} tasks completed</p></div></> : <p className="mt-6 text-sm text-muted">No tasks created in this period yet.</p>}</section>
    </div>
